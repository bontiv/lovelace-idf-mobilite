...
class IDFMobiliteCard extends LitElement {
    static get properties() {
        console.log("%c Lovelace - IDF Mobilité  %c __VERSION__", "color: #FFFFFF; background: #5D0878; font-weight: 700;", "color: #fdd835; background: #212121; font-weight: 700;")
        return {
            hass: {},
            config: {},
        };
        ...
        <ha-card>
            <div class="${this.getBorderClass()}">
                <div class="${this.getScreenClass()}">
                    ...
                    ? html`
                        <div class="ratp-img">
            <img src="${images['ratp.png']}" class="ratp-image">
    ...
    *********************************************************/
    createRERContent(lineDatas, exclude_lines, exclude_lines_ref, nb_departure, show_hour_departure, show_hour_departure_index, show_departure_platform, groupDestination, groupDestinationName, max_delay, show_replacement_bus, second_entity) {
        ...
        if (!model)
            return
         return html`
            ${renderRER(model, this.config, images, second_entity)}
        `;
    }
    /********************************************************
    **                                                     **
    ...
    *********************************************************/
    createBUSContent(lineDatas, exclude_lines, exclude_lines_ref, included_destination, show_only_included, second_entity) {
        ...
        if (!model)
            return
        return html`
            ${renderBUS(model, this.config, images, second_entity)}
        `;
    }
    ...
    *********************************************************/
    createMessageDisplay() {
        ...
        const htmlString = renderMessages(model, this.config, images, noScroll);
